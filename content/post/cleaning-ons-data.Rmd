---
title: Cleaning UK Office for National Statistics data in R with the tidyverse
author: Matt Ashby
date: '2019-08-27'
...
ONS has started publishing some information as what it calls open data, by which 
it means data that is easily machine readable, since (virtually?) all the data 
it produces is already open licenced under the [Open Government
Licence](http://nationalarchives.gov.uk/doc/open-government-licence/version/3/). 
However, most products of its produces are Excel tables that are optimised for 
...
Since the data are in an Excel file, we can use the `readxl` package to read it
into R. ONS publishes data using a mixture of file formats, including both the 
pre-2007 Excel `.xls` file format and the current `.xlsx` format[^1]. The 
`read_excel()` function can handle both `.xls` and `.xlsx` files, but can only 
load a single sheet from within an Excel workbook (i.e. file). We can use the 
`excel_sheets()` function to get the names of each sheet in the Excel file.
...
![Screenshot of the MYE1 sheet as viewed in Excel](mye1-sheet-preview.png)
...
We can fix all of these either using the parameters of `read_excel()` or the 
...
Finally, we will tidy up the data by converting the population variable to 
...
Using the `magrittr` pipe `%>%`, we can combine all these cleaning steps 
together, which makes the code more compact and (arguably) more readable.
...
tidy_data <- read_excel("ons_pop_data.xls", sheet = "MYE1", 
												range = cell_rows(5:31)) %>% 
	inset(1, 1, "group") %>% 
	set_colnames(.[1, ]) %>% 
	slice(2:n()) %>% 
...
use `filter(data, !is.na(population))`. To remove single manually by row number, 
use `slice()`, also from `dplyr`.
...
table as noise that can be removed using a combination of `drop_na()` and 
...
# Some other useful data cleaning functions {#other-functions}

Some other functions that are useful for cleaning ONS data include:

  * `clean_names()` from the `janitor` package, which converts column names to
    lower case with non-alphanumeric characters replaced with underscores;
  * `mutate_at(vars(value1, value2), ~ . / 100)` from `dplyr` to replace (across
    multiple specified columns) percentage values such as prevalence rates with 
    proportions, which are more useful for a variety of operations;
