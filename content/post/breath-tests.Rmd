...
if (!file.exists(paste0(chart_details$id, "-data.csv.gz"))) {
	...
	# convert ODS sheet to CSV
	ods_to_csv(data_file, "BT_01", glue::glue("{data_file}.csv"))

	# read data
	tidy_data <- glue::glue("{data_file}.csv") %>% 
		read_csv(skip = 1) %>% 
		rename(
			year = `Calendar Year`,
			force = `Police Force`,
...
chart_data <- tidy_data %>%
	mutate(
		date = as_date(glue::glue("1 {month} {year}"), tz = "GMT", 
									 format = "%d %B %Y"),
		month_date = as_date(glue::glue("1 {month} 2019"), tz = "GMT", 
									 format = "%d %B %Y")
	) %>% 
	filter(
		date >= ymd("2010-01-01"), 
